import { forceRefreshActiveCredentials, getCachedCredentials, getCredentialsForSync, initAccounts, loadPersistedAccountSource, refreshAccountsList, saveAccountSource, setActiveAccountSource, syncAuthJson, } from "./credentials.js";
import { readAllClaudeAccounts } from "./keychain.js";
import { initLogger, log } from "./logger.js";
import { buildUserAgent } from "./signing.js";
import { injectBillingHeader } from "./transforms.js";
export { getCachedCredentials, syncAuthJson, refreshAccountsList, } from "./credentials.js";
export { readAllClaudeAccounts } from "./keychain.js";
const PROVIDER_ID = "anthropic";
const PROVIDER_LABEL = "Claude Code (subscription)";
const SYNC_INTERVAL = 5 * 60 * 1000; // 5 minutes
function toOAuthCreds(creds) {
    return {
...
const extension = async (pi) => {
    ...
    try {
        ...
        console.warn("pi-claude-auth: Failed to read Claude Code credentials:", error);
        return;
    }
    initAccounts(accounts);
    if (accounts.length === 0) {
        log("extension_init_no_accounts", { reason: "no credentials found" });
        console.warn("pi-claude-auth: No Claude Code credentials found. Run `claude` to authenticate first.");
        return;
    }
    const persistedSource = loadPersistedAccountSource();
    const defaultAccount = (persistedSource &&
    ...
    if (initialCreds) {
        ...
        console.warn("pi-claude-auth: Claude credentials are expired and could not be refreshed. Run `claude` to re-authenticate.");
    }
    ...
    const oauth = {
        ...
        async login(callbacks) {
            ...
            let chosen = latestAccounts.find((a) => a.source === currentSource) ??
                latestAccounts[0];
